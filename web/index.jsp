                    ...
                    <%
                        ...
                        if (session.getAttribute("error") != null) { //comprueba error
                            if (session.getAttribute("error").equals("Usuario o contraseña incorrectos, por favor, inténtelo de nuevo")) { //comprueba el valor de error
                                %><p class="message text-danger"><%out.print(session.getAttribute("error"));%></p><% //mensaje rojo error
                                session.setAttribute("error", "null"); //resetea atributo error
                            }
                            if (session.getAttribute("error").equals("No se ha iniciado sesion")) { //comprueba el valor de error
                                %><p class="message text-danger"><%out.print(session.getAttribute("error"));%></p><% //mensaje rojo error
                                session.setAttribute("error", "null"); //resetea atributo error
                            }
                        }
